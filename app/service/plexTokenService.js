...
module.exports = class plexTokenService {
    ...
    async getPin() {
        ...
        if (p) {
            ...
            const r = { 'code': this._api.getPin(), 'id': this._api.getRequestId(), 'expires': this._api.getExpireTime() };
            return r;
    ...
    async checkPin() {
        ...
        if (r) {
            ...
            if (!this._api.getAuthToken()) throw new Error('Unable to authenticate token');
            return this._api.getAuthToken();
        } else throw new Error('PIN Token expired');
    }
}