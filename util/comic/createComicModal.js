...
{
    let i = ComicItem.results[0].count_of_issues === null ? "No count registered" : ComicItem.results[0].count_of_issues;
    let o = ComicItem.results[0].description === null ? "No description" : ComicItem.results[0].description;
    let t = o.substring(0, 200);
    let r = t.replace(/<(?:.|\n)*?>/gm, '');
