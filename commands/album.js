...
exports.run = (bot, msg, args = []) => {
  ...
  const urlSearch = `http://ws.audioscrobbler.com//2.0/?method=album.search&album=${query}&api_key=cd564d2f0dd91dd49b4e1d655dffd02c&format=json`;
  request(urlSearch, function(error, res2, body) {
    if (!error) {
      ...
      if (mbidInQuery(query, 'mbid')) {
        const urlInfo = `http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=cd564d2f0dd91dd49b4e1d655dffd02c&mbid=${args[1]}&format=json`;
        request(urlInfo, function(error, res, body) {
          if (!error) {
            const info = JSON.parse(body);

            const overview = info.album.wiki.content === null ? 'No description' : info.album.wiki.content;
