...
exports.run = (bot, msg, args = []) => {
  ...
  const urlCorrection = `http://ws.audioscrobbler.com/2.0/?method=artist.getCorrection&artist=${query}&api_key=cd564d2f0dd91dd49b4e1d655dffd02c&format=json`;
  request(urlCorrection, function(error, res2, body) {
    if (!error) {
      const info = JSON.parse(body);
      const correctedArtist = info.corrections.correction.artist.name;
      const url = `http://ws.audioscrobbler.com/2.0/?method=artist.getInfo&artist=${correctedArtist}&api_key=cd564d2f0dd91dd49b4e1d655dffd02c&format=json`;
      request(url, function(error, res2, body) {
        if (!error) {
          const info = JSON.parse(body);

          const onTour = info.artist.onTour === '0' ? 'No' : 'Yes';
          const overview = info.artist.bio.content === null ? 'No description' : info.artist.bio.content;
