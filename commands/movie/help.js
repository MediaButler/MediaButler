...
exports.run = (bot, msg, args = []) => {
  if (!args[0]) {
    ...
    msg.channel.send(`= Command List =\n\n[Use ${settings.prefix}movie help <command> for details]\n\n${bot.movieCommands.map(c => `${settings.prefix}movie ${c.help.name}${' '.repeat(longest - c.help.name.length)} :: ${c.help.description}`).join('\n')}`, { code: 'asciidoc' });
  } else {
    let command = args[0];
